...
RULE: Use code_execution_tool with runtime=terminal. The argument is code (not commands). Put all commands in the code field as a single string.
command_1: <exact shell command>
Return: command_1_output, sentinel_verdict, sentinel_reason
```
IMPORTANT: docker --format with Go templates ({{range .Mounts}}) must use single quotes on remote side: `"docker inspect c --format '{{range .Mounts}}{{.Source}}{{end}}'"`. Without single quotes the remote shell word-splits the template and docker returns "template parsing error: unclosed action". Safer alternative: omit --format and grep the JSON output for "Source".
26. [RESOLVED 2026-06-13 session 13] Agent Zero behavioral loop prevention — Root cause of context 8gqNgsIu infinite retry loop: agent.py had no repeat counter. When qwen2.5-coder:7b emits the same JSON response (e.g., document_artifact search), the runtime appends "same message" warning but does NOT break the loop. Model ignores warning, regenerates same response, loop continues indefinitely. Fix: added repeat_count field to LoopData (agent.py line 335), incremented on each same-message event, hard stop at repeat_count>=3 returning "HARD STOP: Same response repeated 3 times. Task declared BLOCKED." Wedged context 8gqNgsIu terminated via api_reset_chat endpoint (POST /api/api_reset_chat, X-API-KEY header, no CSRF required). Agent Zero restarted to load fix. agent.py commit 294100c6. Two prior runbooks on same root cause: this is now structurally prevented.
25. [RESOLVED 2026-06-13 session 13 cont.] Hermes desk → Agent Zero credential + approval path — docker-compose.yml updated with AGENT_ZERO_DESK_API_KEY=Zu5PK6EMFE5Dd1oE and AGENT_ZERO_DESK_URL=http://host.docker.internal:50080. Hermes desk profile config.yaml updated: approvals.mode=auto (was smart), security.allow_private_urls=true (was false). Hermes restarted successfully. agent-zero-desk SSH verification task confirmed: Agent Zero correctly used code_execution_tool with runtime=terminal and code field, SSHed VM107, obtained docker inspect output showing /mnt/truenas-canonical/hermes-librarian as mount source. Architecture GREEN.
24. [RESOLVED 2026-06-13 session 12] Agent Zero Notion MCP access — 22 authenticated Notion tools now active on agent-zero-desk. Root cause of prior failure: _create_stdio_transport passed server.env dict as-is to StdioServerParameters — §§secret() placeholders were not resolved before subprocess launch. Fix: 3-line secrets resolution step added before StdioServerParameters creation in mcp_handler.py (commit 205d2ea2 in agent-zero-desk-data local repo). Secrets stored in usr/secrets.env (gitignored, recreate from /home/tunedr/CREDENTIALS.env). MCP config in usr/settings.json (gitignored). Verified: API-post-search returns results, API-retrieve-a-page returns Master Hub page, API-patch-block-children appends blocks. 22 tools: API-get-user, API-get-users, API-get-self, API-post-search, API-get-block-children, API-patch-block-children, API-retrieve-a-block, API-update-a-block, API-delete-a-block, API-retrieve-a-page, API-patch-page, API-post-page, and 10 more. document_query retired for Notion — use MCP tools. Runbook: 37e6d271-f21c-81c4-bc22-cb3200e8355f. OPEN: private git remote needed for agent-zero-desk-data so prompt/handler patches can be pushed.
23. [RESOLVED 2026-06-13 session 11] Agent Zero Runbook Mode + Rule 2 RED + sentinel_verdict — Closed all three YELLOW gaps from session 10. (1) Runbook Mode end-to-end verified: Agent Zero logs confirm mode detection ("I received a Notion runbook URL. Let me read the runbook and execute it phase by phase."), Phase 1 SSH executed, Phase 2 call_subordinate called — multi-phase continuation confirmed. (2) Rule 2 tightened: "DOCKER PRUNE IS ALWAYS RED — NO EXCEPTIONS" callout added at top of Rule 2 in both specifics files. Test C confirmed: docker system prune -af --volumes returned RED with explicit Rule 2 citation, NOT executed. (3) sentinel_verdict added to 23-field report schema table (was missing from Session 10's 20-field table). Test A confirmed: all 23 fields including sentinel_verdict:GREEN, sentinel_reason:"Rule 4 auto-pass", sentinel_redesign_feedback:N/A returned in ad-hoc SSH report. Session verdict: GREEN. Both specifics files updated (two-specifics-files doctrine). Runbook: 37e6d271-f21c-81e1-b948-df079a6e0bee.
22. [RESOLVED 2026-06-13 session 10] Agent Zero Task Mode Boundary — Ad-hoc tasks now stop after returning structured report. Root cause: SOUL prompt had no task-mode detection, causing model to continue into runbook-fetch loop after completing ad-hoc tasks. Fix: (1) Added Runbook Mode / Ad-Hoc Mode distinction to both specifics files with explicit Ad-Hoc Stop Rule. (2) Renamed "## Sentinel Validation" section to "## Pre-Execution Rules Check (THINK-ONLY, NO TOOL CALL)" — eliminated qwen2.5-coder:7b hallucination of sentinel_check tool call that was causing infinite loops and curl timeouts. Test A (ad-hoc SSH) confirmed GREEN: HTTP 200, full 20-field report, no post-report loops, pop-ollama uptime returned. Test C (docker system prune): NOT executed, response returned (ORANGE verdict — model reasoning gap, not RED as expected; follow-up item). Runbook: 37e6d271-f21c-8119-a031-e59e73b28845. New doctrine: rename trigger words, not just add prohibitions — text prohibitions alone did not stop qwen2.5-coder:7b from calling sentinel_check.
